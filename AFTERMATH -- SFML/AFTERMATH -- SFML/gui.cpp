...
{
	/*
	* Converts sa percentage value to pixels relative to the current resolution in the x axis.
...
{
	/*
	* Converts sa percentage value to pixels relative to the current resolution in the y axis.
	*  @param    float perc    the percentage value.
	*  @param    sf::Videomode& vm    the current videomode of the window (resolution).
	*	@return	 float			the calculated pixel value
	*/
...
void gui::DropDownBox::update(const sf::Vector2i& mousePosWindow, const float& dt)
{
...
gui::ProgressBar::ProgressBar(float _x, float _y, float _width, float _height, int max_value, sf::Color inner_color, unsigned character_size, sf::VideoMode& vm, sf::Font* font)
{
	...
	if (font)
	{
		...
		this->text.setPosition(this->inner.getPosition().x + gui::p2pX(0.53f, vm), this->inner.getPosition().y + gui::p2pY(0.5f, vm));
	}
}
